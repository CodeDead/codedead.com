...
export const getStaticProps = async ({ params }) => {
  ...
  // End = start + number of posts per page, unless it's the last page
  const end =
    start + numberOfPostsPerPage < posts.length ? start + numberOfPostsPerPage : posts.length;
  const requiredPosts = sortedPosts.slice(start, end);
...
const BlogOverview = ({ posts, currentPage, numberOfPages }) => {
      ...
      <Container size="xl">
        ...
        <Container mt={50}>
          <Pagination
            ...
            total={numberOfPages}
            value={parseInt(currentPage, 10) + 1}
            onChange={setPage}
          />
        </Container>
      </Container>
